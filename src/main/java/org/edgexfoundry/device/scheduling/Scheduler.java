 ...
 * the License.
 *
 * @microservice: device-scheduling
 * @author: Tyler Cox, Dell
 * @version: 1.0.0
 *******************************************************************************/

package org.edgexfoundry.device.scheduling;
...
public class Scheduler {
  ...
  @Autowired
  private ScheduleConfiguration defaultSchedules;
  ...
  public void addDefaultSchedules() {
    ...
    for (int i = 0; i < defaultSchedules.getSize(); i++) {
      String name = defaultSchedules.getNames()[i];
      String start = defaultSchedules.getStarts()[i];
      String end = defaultSchedules.getEnds()[i];
      String frequency = defaultSchedules.getFrequencys()[i];
      String cron = defaultSchedules.getCrons()[i];
      Boolean runOnce = Boolean.valueOf(defaultSchedules.getRunOnces()[i]);
  ...
  public void addDefaultScheduleEvents() {
    for (int i = 0; i < defaultScheduleEvents.getSize(); i++) {
      String name = defaultScheduleEvents.getNames()[i];
      String path = defaultScheduleEvents.getPaths()[i];
      String service = defaultScheduleEvents.getServices()[i];
      ...
      String schedule = defaultScheduleEvents.getSchedules()[i];

      ScheduleEvent scheduleEvent =
          new ScheduleEvent(name, addressable, parameters, schedule, service);
